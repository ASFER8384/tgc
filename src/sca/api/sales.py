...
from sca.models import AuditLog, Item, StockLevel, StockSnapshot
...
async def record_sale(body: SaleIn, session: SessionDep, actor: ActorDep) -> SaleOut:
    ...
    if body.move_stock:
        stock_out, notes = await _apply_stock(session, merged, occurred=occurred)
    else:
        stock_out, notes = [], [
...
async def _apply_stock(
    session, merged: dict[str, SaleLine], *, occurred: datetime
) -> tuple[list[dict], list[str]]:
    """Take the sold units off the shelf, and append the new position.

    The ledger append is the point of the whole exercise: ``planning.demand``
    divides by the weeks an item was sellable, and it reads that from these rows.
    A sale that moved stock without leaving a reading behind would still be
    counted as demand while contributing nothing to the divisor it belongs to.
    """
    out: list[dict] = []
    notes: list[str] = []
    for sku, line in sorted(merged.items()):
        ...
        if after < 0:
            # The record was wrong before this sale, not because of it. Held at
            # zero rather than allowed to go negative: a negative on-hand reads
            # downstream as "not sellable", which would shrink the divisor in the
            # demand calculation and inflate the very figure this is meant to
            # measure — the shelf would end up buying more of itself.
            notes.append(
                f"{sku}: sold {line.quantity} but only {before} were on record. "
                "Held at 0 — the count needs checking."
            )
            after = 0
        snapshot.on_hand = after
        session.add(
            StockLevel(
                sku=sku, on_hand=after, on_order=snapshot.on_order, recorded_at=occurred
            )
        )
        out.append({"sku": sku, "sold": line.quantity, "was": before, "now": after})
    return out, notes
